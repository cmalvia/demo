...
stage('Scan image with twistcli') {
        //withCredentials([usernamePassword(credentialsId: 'twistlock_creds', passwordVariable: 'TL_PASS', usernameVariable: 'TL_USER')]) {
            sh 'curl -k -u admin:P@ssw0rd --output ./twistcli https://10.129.130.59:8083/api/v1/util/twistcli'
            sh 'chmod a+x ./twistcli'
            sh "./twistcli images scan -u admin -p P@ssw0rd --address https://10.129.130.59:8083 cmalvia/my-app* --details"
}
...
## work and tested on Local setup **
